...
begin
	...
	begin
		...
		if (ret==PRINT) then print();
		else done=TRUE;	/* EXIT because only other exit button	*/
...
dump_card()
	begin
		...
		device=AUX;
		if ((Setprt(-1)&16)==0) then device=PRT;
		...
		begin
			...
			begin
				...
				data= card[i][j] & 0xff;
				if (last_control!=control) then
				begin
					if (control==UNDER) then
					begin /*underline on*/
						Bconout(device,27);
						Bconout(device,'-');
						Bconout(device,1);
					end
					if (control==BOLD) then
					begin /*bold on*/
						Bconout(device,27);
						Bconout(device,'E');
					end
					if (last_control==UNDER) then
					begin /*underline off*/
						Bconout(device,27);
						Bconout(device,'-');
						Bconout(device,0);
					end
					if (last_control==BOLD) then
					begin /*bold off*/
						Bconout(device,27);
						Bconout(device,'F');
					end
...
copy_card(print_flags,string1,string2)
	...
	begin
		...
		begin
			flag=print_flags;
			if ((flag==UNDER) && (string2[i]==' ')) then flag=0;
