...
static int wr_mode, txt_point, txt_color, halign, valign,
    ln_pat, ln_type=-1, ln_color, f_style, f_color, f_inter, effects,
    first_char, ch_mode;
static char NVDI3, true_type;
...
int _vst_font( int id, int scale )
{
  if( gr_font != id )
  {
    ...
    if( (gr_font = vst_font( vdi_hand, id )) != id ) is_scalable = 0;
    else if( (is_scalable = scale) != 0 ) first_char = id<0 ? 32 : 0;
    txt_point = -1;
