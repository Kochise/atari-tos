 ...
 if (vm_in_use)
 {
  ...
  if (((page_size+st_pagesize)>0) &&((page_size+st_pagesize)<=0x02000000))
  {
   ...
   if (table_mem=pagesave)
   {
    ...
    /* originally the lowest VM is in mem the rest is on the partition */    
   }
   else
   {
    /* tt-ram too large */
 ...
 */
	if (rfreelist && !rfreelist->next) {
		...
		if (newfrees) {
			...
			newfrees[NEWREGIONS-1].shadow = 0;
			newfrees[NEWREGIONS-1].save = 0;
	...
	MMAP map;
{
	...
	while (m) {
	    FORCE("%ld bytes at %lx (%d links, mflags %x); next %lx",
		    m->len, m->loc, m->links, m->mflags, m->next);
	    if (m->shadow)
	...
	MMAP map;
{
	...
	while (m) {
		...
		if (nxt) {
			...
			else if (m->loc == nxt->loc && m->len > 0
				 && (m->shadow != nxt || m->len != nxt->len)) {
