 ...
 */
	...
	TRACE(("p_vfork: parent going to sleep, wait_cond == %lx",
		(long)p));
 ...
 * prevent swapping of the region and its save region (which should still
 * be identical) on the next context switch, we attach them here to the
 * current process.
 */
	for (i = 0; i < curproc->num_reg; i++) {
		m = curproc->mem[i];
...
/* Correct the parent basepage pointer on the child, it stills points to
 * its grandparent's basepage.
 */
...
/* Now run the child process. We want it to run ASAP, so we temporarily
 * give it high priority and put it first on the run queue. Warning:
 * After the yield() the "p" structure may not exist any more.
 */
	run_next(p, 3);
	newpid = p->pid;
	yield();
