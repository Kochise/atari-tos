...
struct blit_form_t {
    int16_t  xmin;  /* +14/+28 minimum X: source */
    int16_t  ymin;  /* +16/+30 minimum Y: source */
    uint16_t *form; /* +18/+32 source form base address */
    int16_t  nxwd;  /* +22/+36 offset to next word in line  (in bytes) */
    int16_t  nxln;  /* +24/+38 offset to next line in plane (in bytes) */
    int16_t  nxpl;  /* +26/+40 offset to next plane from start of current plane */
};

/* 76-byte line-A BITBLT struct passing parameters to bitblt */
struct __attribute__ ((__packed__)) blit_frame /* to avoid structs being on 32 bits boundary*/ {
    ...
    struct blit_form_t s;  /* +14 source form definition */
    struct blit_form_t d;  /* +28 destination form definition */
    struct {
        uint16_t *addr;  /* +42 address of pattern buffer   (0:no pattern) */
        int16_t nxln;      /* +46 offset to next line in pattern  (in bytes) */
        int16_t nxpl;      /* +48 offset to next plane in pattern (in bytes) */
        int16_t mask;      /* +50 pattern index mask */
    } pattern;
...
void vr_trnfm(const MFDB *src_mfdb, MFDB *dst_mfdb)
{
    ...
    int16_t n_planes;   /* Number of planes */
    bool inplace;       /* Whether source and destination memory blocks overlap */
    int32_t size;       /* Size of the source, in words */
    int32_t inner, outer, i, j;

    src = src_mfdb->fd_addr;
    dst = dst_mfdb->fd_addr;
    n_planes = src_mfdb->fd_nplanes;
    inplace = (src == dst);
    size = (int32_t)src_mfdb->fd_h * src_mfdb->fd_wdwidth; 

    if (src_mfdb->fd_stand)
    {
        /* Source is in standard format */
        dst_mfdb->fd_stand = 0; /* Force dest to device-dependent */
        outer = n_planes;       /* Set outer & inner loop counts  */
        inner = size;
    }
    else
    {
        /* Source is in device-dependent format */
        dst_mfdb->fd_stand = 1; /* Force dest to standard */
        outer = size;           /* Set loop counts */
        inner = n_planes;
    }

    if (!inplace) /* The simplest option */
    {
    ...
    if (n_planes == 1)            /* for mono, there is no difference    */
...
void vdi_vrt_cpyfm(int16_t handle, int16_t vr_mode, vdi_rectangle_t *src_dst, MFDB *psrcMFDB, MFDB *pdesMFDB, int16_t *colors);
{
    ...
    vdi_info.pattern.addr = vwk->patptr;
     ...
     int16_t mode, int16_t *colors)
{
    ...
    /* mode is invalid if made up of more than the first 5 bits */
    if (mode & ~0x001f)
        return;

    /* check the pattern flag (bit 5) and revert to log op # */
    info->pattern.addr = 0L;  /* get pattern pointer */
    if (mode & PAT_FLAG) {
    ...
    if (!raster->transparent) {
        ...
        info->s.nxpl = 0;       /* use only one plane of source */
...
{
    /* multi-plane pattern? */
    info->pattern.nxpl = 0;           /* next plane pattern offset default. */
    if (raster->multifill) {
        info->pattern.nxpl = 32;      /* yes, next plane pat offset = 32. */
    }
    info->pattern.nxln = 2;        /* offset to next line in pattern */
    info->pattern.mask = 0xf;      /* pattern index mask */
}
...
static bool setup_info(struct raster_t *raster, struct blit_frame * info, MFDB *src, MFDB *dst)
{
    ...
    if (src->fd_addr) {
        ...
        info->s.form = src->fd_addr;
        info->s.nxwd = src->fd_nplanes * 2;
        info->s.nxln = src->fd_wdwidth * info->s.nxwd;
    }
    else {
        /* source form is screen */
        info->s.form = (uint16_t*)v_bas_ad;
        info->s.nxwd = raster->n_planes * 2;
        info->s.nxln = raster->line_length;
    }

    /* setup plane info for destination MFDB */
    if (dst->fd_addr) {
        /* for a positive address */
        info->d.form = dst->fd_addr;
        info->plane_ct = dst->fd_nplanes;
        info->d.nxwd = dst->fd_nplanes * 2;
        info->d.nxln = dst->fd_wdwidth * info->d.nxwd;
    }
    else {
        /* destination form is screen */
        info->d.form = (uint16_t*)v_bas_ad;
        info->plane_ct = raster->n_planes;
        info->d.nxwd = raster->n_planes * 2;
        info->d.nxln = raster->line_length;
    ...
    if (use_clip) {
        ...
        dont_clip(info);
#endif
    info->s.nxpl = 2;           /* next plane offset (source) */
    info->d.nxpl = 2;           /* next plane offset (destination) */
...
static void bit_blt(struct blit_frame *blit_info)
{
    ...
    s_xmin = blit_info->s.xmin;               /* d0<- src Xmin */
    s_xmax = s_xmin + blit_info->b_wd - 1;    /* d1<- src Xmax=src Xmin+width-1 */
    d_xmin = blit_info->d.xmin;               /* d2<- dst Xmin */
    ...
    s_addr = (uint32_t)blit_info->s.form
        + (uint32_t)blit_info->s.ymin * (uint32_t)blit_info->s.nxln
        + (uint32_t)s_xmin_off * (uint32_t)blit_info->s.nxwd;
    d_addr = (uint32_t)blit_info->d.form
        + (uint32_t)blit_info->d.ymin * (uint32_t)blit_info->d.nxln
        + (uint32_t)d_xmin_off * (uint32_t)blit_info->d.nxwd;
    ...
    if ((s_addr < d_addr)
     || ((s_addr == d_addr) && (skew >= 0))) {
        ...
        s_addr = (uint32_t)blit_info->s.form
            + (uint32_t)blit_info->s_ymax * (uint32_t)blit_info->s.nxln
            + (uint32_t)s_xmax_off * (uint32_t)blit_info->s.nxwd;
        d_addr = (uint32_t)blit_info->d.form
            + (uint32_t)blit_info->d_ymax * (uint32_t)blit_info->d.nxln
            + (uint32_t)d_xmax_off * (uint32_t)blit_info->d.nxwd;

        /* offset between consecutive words in planes */
        blt->src_x_inc = -blit_info->s.nxwd;
        blt->dst_x_inc = -blit_info->d.nxwd;

        /* offset from last word of a line to first word of next one */
        blt->src_y_inc = -(blit_info->s.nxln - blit_info->s.nxwd * s_span);
        blt->dst_y_inc = -(blit_info->d.nxln - blit_info->d.nxwd * d_span);
    ...
    for (plane = 0; plane < blit_info->plane_ct; plane++) {
        ...
        s_addr += blit_info->s.nxpl;          /* a0-> start of next src plane   */
        d_addr += blit_info->d.nxpl;          /* a1-> start of next dst plane   */
    }
}
